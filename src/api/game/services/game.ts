      ...
      const game = await strapi.service(gameService).create({
        data: {
          ...
          categories: await Promise.all(
            product.genres.map(({ name }) => getByName(name, catergoryService)),
          ),
          platforms: await Promise.all(
            product.operatingSystems.map((name) => getByName(name, platformService)),
          ),
          developers: await Promise.all(
            product.developers.map((developer) => getByName(developer, developerService)),
          ),
          publisher: await Promise.all(
            product.publishers.map((publisher) => getByName(publisher, publisherService)),
          ),
